          ...
          CupertinoPageRoute(builder: (_) => OtpWidget(
            callback: (code)async{
              if(otpCode=="$code"){
                ...
                final resRegister = await BaseController().post(url: "auth/signup",data: dataRegister,context: context);
                if(resRegister!=null){
                  final dataUser={
                    "${UserTable.idUser}":resRegister["data"]["id"].toString(),
                    "${UserTable.token}":resRegister["data"]["token"].toString(),
                    "${UserTable.fullname}":resRegister["data"]["fullname"].toString(),
                    "${UserTable.mobile_no}":resRegister["data"]["mobile_no"].toString(),
                    "${UserTable.photo}":resRegister["data"]["photo"].toString(),
                    "${UserTable.cover}":resRegister["data"]["cover"].toString(),
                    "${UserTable.email}":resRegister["data"]["email"].toString(),
                    "${UserTable.referral}":resRegister["data"]["referral"].toString(),
                    "${UserTable.status}":resRegister["data"]["status"].toString(),
                    "${UserTable.statusRoleApp}":"${StringConfig.masukKeAplikasi}"
                  };
                  final user = Provider.of<UserController>(context,listen: false);
                  final checkUser=await db.getData(UserTable.TABLE_NAME);
